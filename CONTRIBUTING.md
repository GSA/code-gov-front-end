...
## How to contribute to code-gov-front-end
...
- [Share your ideas](mailto://code@gsa.gov) for how to improve code-gov-front-end. We're always open to suggestions.
...
- Improve or create documentation for code-gov-front-end.
...
# Issue Guidelines
In the interest of making it easier for our developers to manage site issues, we request that issues follow these guidelines.  Our issues guidelines are a living document and a work in progress, so feel free to offer feedback on it, by emailing us at code@gsa.gov
 ...
 - **Measurable**: The issue has specific requirements and can be closed when those requirements are met.  This helps us track the progress in address problems.  For example, tech debt is always something to work on, so we should create separate issues for this, such as `Consolidate Styling of Repo and Task Cards` and `Convert Validator from Monaco Editor to jsoneditor`, rather than `Address Tech Debt`.
 - **Actionable**: The issues requests a specific action from a member of the code.gov team or community.  An example of something actionable would be `Remove Debugging Text from Loading Screen`.  An example of something that isn't actionable by our team or community is `My city should be forking Code.gov`.
 ...
 - **Timed**: The issue is something that can be resolved within a year.  If you think your issue might take longer, we encourage you to break up your issues into smaller issues (like stepping stones) that we can address.
