...
describe('utils - url-parsing', () => {
  ...
  describe('getSection', () => {
    it('should return `browse` if the pathname includes `/browse-projects`', () => {
      window.history.pushState({}, '', 'http://test.com/browse-projects?a=123&b=test');
      expect(getSection()).toEqual('browse');
      window.history.pushState({}, '', '/browse-projects/test-url.png');
      expect(getSection()).toEqual('browse');
    });

    it('should return `search` if the pathname includes `/search`', () => {
      window.history.pushState({}, '', 'http://test.com/search?a=123&b=test');
      expect(getSection()).toEqual('search');
      window.history.pushState({}, '', '/search/test-url.png');
      expect(getSection()).toEqual('search');
    });

    it('should return `tasks` if the pathname includes `/open-tasks`', () => {
      window.history.pushState({}, '', 'http://test.com/open-tasks?a=123&b=test');
      expect(getSection()).toEqual('tasks');
      window.history.pushState({}, '', '/open-tasks/test-url.png');
      expect(getSection()).toEqual('tasks');
    });

    it('should return `project` if the pathname includes `/projects/`', () => {
      window.history.pushState({}, '', 'http://test.com/projects/project-1?a=123&b=test');
      expect(getSection()).toEqual('project');
      window.history.pushState({}, '', '/projects/test-url.png');
      expect(getSection()).toEqual('project');
    });

    it('should return falsy for unmatched pathnames', () => {
      window.history.pushState({}, '', 'http://test.com/no-match?a=123&b=test');
